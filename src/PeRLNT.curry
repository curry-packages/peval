...
getOpts :: PEM Options
getOpts = fmap pesOptions get

lookupRule :: QName -> PEM (Maybe Rule)
lookupRule f
  = do s <- get
       let mbFunc = find (hasName f) (pesDecls s)
...
incrRenamingIndex j =
  get >>= \s ->
  let i = pesFresh s
  in put s { pesFresh = i + j } >> return i

incrDepth :: PEM ()
incrDepth = modify $ \ s -> s { pesSteps = pesSteps s + 1 }

-- local criteria if we can proceed unfolding a call in a given state
proceed :: PEM Bool
proceed = fmap (\s -> pesSteps s < 1) get
...
traceM msg = getOpts  >>= \opts ->
             getTrace >>= \t    ->
             return (traceDetail opts (replicate (2 * t) ' ' ++ msg) ())

--- Get current nesting depth.
getTrace :: PEM Int
getTrace = fmap pesTrace get

--- Nest the next invocation by increasing the nesting depth.
nestTrace :: PEM a -> PEM a
nestTrace act = modify (\s -> s { pesTrace = pesTrace s + 1 }) >>
                act >>= \res ->
                modify (\s -> s { pesTrace = pesTrace s - 1 }) >>
                return res
...
peLet ds e = case e of
    ...
    Comb FuncCall _ _
      ...
      -- (LET-SQ)
      Just e' -> return $ topSQ (Let ds e') -- peval (Let ds e')
      _       -> letEval
      -- (LET-PART)
...
peFree vs e = peval e >>= \e' ->
              ...
              in  if e' /= e then peval free' else return free'
...
peCase ct bs subj = case subj of
  ...
  Comb FuncCall _ _
    ...
    | otherwise              -> case getSQ subj of
      -- (CASE-SQ)
      Just e -> return $ topSQ (Case ct e bs) -- peval (Case ct e bs)
      _      -> caseEval
  -- (CASE-ERROR)
